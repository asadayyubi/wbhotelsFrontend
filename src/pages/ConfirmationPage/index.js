...
const ConfirmationPage = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`https://api.ratebotai.com:8443/get_booking_data?booking_id=${location?.state?.data?.booking_id || booking_id}&custumer_id=${location?.state?.data?.customer_id || custumer_id}`).then((response) => {
        ...
        if (bookInfo.booking_status === 'confirmed') {
            return (
                <div>
                    ...
                    <div className="confirm-sub-text">You will soon receive an email confirmation on {bookInfo.email}</div>
                    <div className="print-button">
                        ...
                        <ButtonPrimary text="Print" link={`https://api.ratebotai.com:8443/booking_pdf?booking_id=${bookInfo.booking_id}&customer_id=${bookInfo.customer_id}`}/>
